...
| `InteractiveNeuron` | `src/components/InteractiveNeuron.tsx` | default, firing, subthreshold, reduced-motion | Interactive Nervous System hero. SVG neuron on `bg-ink-900`/`border-line-700`. A stimulus slider + "Stimulate" button fire an all-or-nothing action potential (sand pulse) down a myelinated axon, release neurotransmitter (sand dots) across the synapse, and charge the downstream neuron (sage glow); below threshold the soma flashes and fizzles. rAF-driven; honors `prefers-reduced-motion` by snapping to the result. |
